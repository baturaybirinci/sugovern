...
const CreateChildDAO = ({onCreateChildDAO}) => {
    const daoInfo = useRef({name: "", image:"",desc: "", YKTokenName: "", YKTokenSymbol: "", voterTokenName: "", voterTokenSymbol: ""});
    return (
    <div className='container border border-black p-4'>
        ...
        <form>
            ...
            <br/><br/>
            <button type="button" onClick={() => { onCreateChildDAO(daoInfo.current["name"], daoInfo.current['image'],daoInfo.current["desc"], daoInfo.current["YKTokenName"], daoInfo.current["YKTokenSymbol"], daoInfo.current["voterTokenName"], daoInfo.current["voterTokenSymbol"])}} className="btn btn-primary">Submit</button>
        </form>
    </div>
  )
}
