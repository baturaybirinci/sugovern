...
export default function Dao() {
            ...
            if (!contracts["daoFactoryContract"]) {
                ...
                if (!contracts["daoContract"]) {
                    DaoIsExist(address).then((res) => {
                        ...
                        contracts["daoFactoryContract"].methods
    ...
    const getSubDAOs = async () => {
        ...
        for (var i = 0; i < numChildren; i++) {
            await contracts.daoFactoryContract.methods
                .parent_child_daos(String(address), parseInt(i))
                .call()
                .then((result) => {
                    subDAOs.push(result)
                })
    ...
    const getDaoImage = async (address) => {
        if (!initialized) {
            await init()
        }
    ...
    const createChildDAOFunc = async (
        ...
        await contracts.daoFactoryContract.methods
            .createChildDAO(
                ...
                String(voterTokenName),
                String(voterTokenSymbol)
            )
            .send({
                from: walletAddress,
            })
